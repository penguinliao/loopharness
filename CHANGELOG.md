...
任何一道不过即短路（后面的门不执行，静态能拦的不烧 LLM 额度）：

1. 自审判定（PROCEED/FAIL 严格匹配）
2. ruff + mypy
3. **二审：Claude 干净上下文**（`claude -p` 单次调用，审查者不知道代码怎么写出来的）
4. 三审：DeepSeek 跨家族（配 key 才启用）

二审约束照搬 v1.1.4 救活三审的全套（G4 0/38 教训）：一轮出结果、只看 diff、
看不到的不许编、fail-open（CLI 缺失/超时/输出无法解析 → 放行不阻塞）。
FAIL → 自动回炉，原因进错题本。

关键实现细节：二审子进程 `cwd` 指向系统临时目录——(1) 防止它加载被审项目自己的
hooks（stop_check 会拦它收工导致 180s 超时）；(2) 审查者物理上读不到仓库，
"只看 diff"从软规则变硬约束。这个 bug 是自审阶段发现的，回炉 1 次修复（错题本
机制在本次开发中真实工作了一轮）。

### 修复

- `_cross_family_review` FAIL 消息改含"判定不通过"——此前措辞匹配不到 cmd_advance
  的回炉关键词，三审 FAIL 会卡死在 REVIEW 阶段（hook 锁代码改不了，advance 又过不去）

### 吃狗粮记录

本版二审上线后第一单生意就是审查"实现它自己"的 diff——真实调用 `claude -p`，
判定 PROCEED。19/19 AC 测试全过（含 v1.2.0 的 12 个回归）。
