...
> **把 AI "loop engineering"（循环工程）的最佳实践，打包成非技术产品经理也能用的一句话。**

> 300 行 Python，让 Claude Code 在听不懂的需求面前也能写对代码——并且自己发现错、自己回炉、自己记住教训，直到真的做对。**你不用懂代码，只管说要做什么。**
...
**🌙 v1.2+ 无人值守**：晚上对 Claude Code 说一句 `/loop 把这个功能做完，做完或卡住叫我`，然后去睡觉。你的代码会被三个互相不通气的审查者看过——自审、Claude 干净上下文二审、DeepSeek 跨家族三审——任何一道 FAIL 都带着错题本自动回炉，不重复失败过的改法。醒来只有三种结局：**done ✓** / **done ⚠️ 带病交付**（附诚实的遗留问题清单）/ **stuck**（等你拍板）。永不挂起，永不糊弄。[→ 无人值守模式](#v12-新增无人值守模式loop)
...
## 为什么说这是一个 "loop engineering"（循环工程）工具

2025–2026 业界逐渐形成共识：AI agent 写代码的好坏，关键不在模型多强，而在**"循环"设计得好不好**——让 AI 行动 → 看结果 → 自我纠错 → 再来一遍，直到真正达标。这门手艺被称为 **loop engineering**。它公认的四道护栏，以及 Claude H-H 的对应实现：

| loop engineering 护栏 | Claude H-H 怎么做 | 给 PM 的好处 |
|---|---|---|
| **① 自我纠错循环** | 测试 / 审查不过 → 自动回炉重改（retreat，最多 3 次） | 你不用来回看代码、催 AI |
| **② 记住教训、不重复犯错** | **错题本物理注入**：每次失败原因直接呈现在 AI 眼前，不靠它"自觉"去读日志 | 无人值守时 AI 也不会重复踩同一个坑 |
| **③ 无进展检测** | **原地打转检测**：连续卡在同一个失败上就提前喊停，不耗满重试预算 | 不烧冤枉钱在死路上反复试 |
| **④ 成本护栏** | **审查调用成本遥测**：`harness status` 看得见每条任务烧了几次 LLM 审查，超额软提醒 | 花费透明可控 |

> 这些护栏不是 PPT 概念——它们是真跑在 hook 和 pipeline 里的**物理约束**（参见下文"无人值守模式"）。

**最关键的是：这一切非技术 PM 也能用。** 你不打开编辑器、不写一行代码，只说一句"做个 X 功能"，或晚上丢一句 `/loop 把它做完，做完或卡住叫我`——剩下的循环，Claude H-H 替你跑完，并对你说人话。
...
- [x] v1.3+：**loop engineering 护栏强化**——错题本物理注入（失败摘要直推 AI）、原地打转/无进展检测（连续卡同一错提前停）、审查调用成本遥测（花费可见 + 软上限提醒）
...
AI 会自己安排节奏循环推进：写规格 → 写代码 → 自审 → 静态检查 → **二审**（Claude 干净
上下文，`claude -p` 单次调用——审查者不知道代码怎么写出来的，没有自我说服偏误）→
**三审**（DeepSeek 跨家族，配了 key 才启用）→ 测试 → 黑盒。任何一道 FAIL 都带着错题本
自动回炉。你睡觉时，代码已经被三个互相不通气的审查者看过了。

> 二审就是 PM 们手动做的"开个新对话让 Claude 独立审查"——现在 pipeline 自动做，
> 无人值守时也不漏。约束与三审相同：一轮、只看 diff、不许编造、失败不阻塞（fail-open）。
